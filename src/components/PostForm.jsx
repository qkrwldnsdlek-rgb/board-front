...
function PostForm() {
  ...
  const handleSubmit = () => {
    ...
    if (isEdit) {
      api.put(`/posts/${id}`, form).then(() => navigate(`/posts/${id}`));
    } else {
      api.post('/posts', form).then(() => navigate('/'));
    }
  };
    ...
    <div style={{maxWidth: '700px', margin: '50px auto', padding: '0 24px'}}>
      <div style={{backgroundColor: '#fff', borderRadius: '20px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)', padding: '40px'}}>
        ...
        <div style={{display: 'flex', gap: '10px', justifyContent: 'flex-end'}}>
          ...
          <button onClick={handleSubmit} style={{backgroundColor: '#5c6bc0', color: '#fff', fontWeight: '600', padding: '10px 24px'}}>
            {isEdit ? '수정 완료' : '등록'}
          </button>
        </div>
      </div>
