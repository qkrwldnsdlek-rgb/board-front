  ...
  return (
        <div style={{ maxWidth: 900, margin: '0 auto', padding: isMobile ? '0 16px' : '0 24px' }}>
              ...
              <div style={{ marginBottom: 24 }}>
                      <h2 style={{ fontSize: 15, fontWeight: 700, color: '#333', marginBottom: 12 }}>카테고리</h2>h2>
                      ...
                      <div
                                    ...
                                    >
                                    <div style={{ fontSize: 18 }}>{cat.label.split(' ')[0]}</div>div>
              ...
              <div style={{ display: 'flex', gap: 20, flexDirection: isMobile ? 'column' : 'row' }}>
                      ...
                      <div style={{ flex: 2 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                                            <h2 style={{ fontSize: 15, fontWeight: 700, color: '#333', margin: 0 }}>🕐 최신 게시글</h2>h2>
                                            <span onClick={() => navigate('/posts')} style={{ fontSize: 12, color: '#5c6bc0', cursor: 'pointer', fontWeight: 600 }}>전체보기 →</span>span>
                                </div>div>
                                <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                                  {loading ? (
                        <div style={{ padding: 24, textAlign: 'center', color: '#999' }}>불러오는 중...</div>div>
                      ) : recentPosts.length === 0 ? (
                        <div style={{ padding: 24, textAlign: 'center', color: '#999' }}>게시글이 없습니다.</div>div>
                        ...
                        <div
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                          <div style={{ flex: 1, minWidth: 0 }}>
                                                                              ...
                                                                              <span style={{ fontSize: 13, fontWeight: 500, color: '#222' }}>{post.title}</span>span>
                                                          </div>div>
                                                          <span style={{ fontSize: 11, color: '#aaa', marginLeft: 8, flexShrink: 0 }}>{formatDate(post.createdAt)}</span>span>
                                        </div>div>
                                        <div style={{ marginTop: 4, fontSize: 12, color: '#999' }}>{post.author} · 👁 {post.viewCount ?? 0}</div>div>
                        </div>div>
                      ))}
                                </div>div>
                      </div>div>
                      ...
                      <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                                            <h2 style={{ fontSize: 15, fontWeight: 700, color: '#333', margin: 0 }}>📢 공지사항</h2>h2>
                                            <span onClick={() => navigate('/posts?category=%EA%B3%B5%EC%A7%80%EC%82%AC%ED%95%AD')} style={{ fontSize: 12, color: '#e53935', cursor: 'pointer', fontWeight: 600 }}>전체보기 →</span>span>
                                </div>div>
                                <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                                  {loading ? (
                        <div style={{ padding: 24, textAlign: 'center', color: '#999' }}>불러오는 중...</div>div>
                      ) : noticePosts.length === 0 ? (
                        <div style={{ padding: 24, textAlign: 'center', color: '#999' }}>공지사항이 없습니다.</div>div>
                        ...
                        <div
                                          ...
                                          onMouseLeave={e => e.currentTarget.style.background = '#fff'}
                                        >
                                        <div style={{ fontSize: 13, fontWeight: 500, color: '#222', marginBottom: 4 }}>{post.title}</div>div>
                                        <div style={{ fontSize: 11, color: '#aaa' }}>{formatDate(post.createdAt)}</div>div>
                        </div>div>
                      ))}
                                </div>div>
                      </div>div>
