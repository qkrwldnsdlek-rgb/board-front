...
// ✅ Fix 1: ReplyInput을 PostDetail 바깥으로 분리
//    → 리렌더링마다 새 컴포넌트로 인식되어 포커스 튀는 버그 해결
    ...
    <div style={{ flex: 1 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px' }}>
        ...
        <button
          onClick={onSubmit}
          style={{
            backgroundColor: replyInputText.trim() ? '#5c6bc0' : '#e0e0e0',
            color: replyInputText.trim() ? '#fff' : '#999',
...
function PostDetail() {
  ...
  // ✅ Fix 2: useCallback으로 감싸서 의존성 안정화
  const loadComments = useCallback(async () => {
    ...
    if (userIds.length > 0) {
      ...
      // ✅ Fix 3: 댓글이 없거나 userId가 없으면 profilesMap 초기화
      setProfilesMap({});
    }
  }, [id]);
