    ...
    <div style={{ flex: 1 }}>
      <input
        ...
        style={{
          ...
          fontSize: '14px', boxSizing: 'border-box', backgroundColor: 'transparent'
        }}
        autoFocus
      />
...
function PostDetail() {
  ...
  const getAvatar = (author, avatarUrl) => (
    avatarUrl ? (
      <img src={avatarUrl} alt={author} style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
    ) : (
      <div style={{ width: 36, height: 36, borderRadius: '50%', backgroundColor: '#5c6bc0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, color: '#fff', fontWeight: 700, flexShrink: 0 }}>
  ...
  const ActionButtons = ({ item, rootCommentId, isRoot = false }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      {user && (
        <button onClick={() => { setActiveReplyId(item.id); setReplyInputText(isRoot ? '' : `@${item.author} `); setReplyTargetId(item.id); }} style={{ backgroundColor: 'transparent', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '700', color: '#3f3f3f', padding: '6px 12px', borderRadius: '20px' }}>답글</button>
      )}
      {user && (user.id === item.userId || user.email === ADMIN_EMAIL) && (
        <>
          <button onClick={() => { setEditingId(item.id); setEditInput(item.content); }} style={{ backgroundColor: 'transparent', border: 'none', cursor: 'pointer', fontSize: '13px', color: '#888', padding: '6px 8px', borderRadius: '20px' }}>수정</button>
          <button onClick={() => handleCommentDelete(item.id)} style={{ backgroundColor: 'transparent', border: 'none', cursor: 'pointer', fontSize: '13px', color: '#e57373', padding: '6px 8px', borderRadius: '20px' }}>삭제</button>
        </>
      )}
    </div>
  );
      ...
      <div style={{ display: 'flex', gap: '12px', position: 'relative' }}>
        ...
        <div 
          ...
          style={{ 
            ...
            backgroundColor: isNew ? 'rgba(92, 107, 192, 0.1)' : 'transparent' 
          }}
        >
          ...
          {editingId === item.id ? (
            <div style={{ marginBottom: '8px' }}>
              <input value={editInput} onChange={e => setEditInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCommentEdit(item.id)} style={{ width: '100%', padding: '6px 0', border: 'none', borderBottom: '2px solid #5c6bc0', outline: 'none', fontSize: '14px', boxSizing: 'border-box', backgroundColor: 'transparent' }} autoFocus />
          ...
          {isRoot && hasChildren && (
            <button onClick={() => setShowReplies(prev => ({ ...prev, [item.id]: !prev[item.id] }))} style={{ backgroundColor: 'transparent', border: 'none', cursor: 'pointer', color: '#5c6bc0', fontWeight: '700', fontSize: '14px', padding: '8px 0', display: 'flex', alignItems: 'center', gap: '4px' }}>
    ...
    <div style={{ maxWidth: '1100px', margin: '50px auto', padding: '0 24px' }}>
      <div style={{ backgroundColor: '#fff', borderRadius: '20px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)', padding: '40px' }}>
        ...
        {post.imageUrl && (
          <>
            <div style={{ marginBottom: '32px' }}>
              <img src={post.imageUrl} alt="첨부 이미지" onClick={() => setShowModal(true)} style={{ maxWidth: '100%', maxHeight: '500px', objectFit: 'contain', borderRadius: '12px', cursor: 'zoom-in', boxShadow: '0 2px 12px rgba(0,0,0,0.1)' }} />
            </div>
            {showModal && (
              <div onClick={() => setShowModal(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out' }}>
        ...
        {post.youtubeUrl && getYoutubeEmbedUrl(post.youtubeUrl) && (
          <div style={{ marginBottom: '32px' }}>
            <iframe key={post.youtubeUrl} width="100%" height="450" src={getYoutubeEmbedUrl(post.youtubeUrl)} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ borderRadius: '12px' }} />
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
          ...
          {user && (user.id === post.userId || user.email === ADMIN_EMAIL) && (
            <>
              <button onClick={() => navigate(`/posts/${id}/edit${fromAdmin ? '?from=admin&tab=posts' : categoryParam}`)} style={{ backgroundColor: '#e8eaf6', color: '#5c6bc0', fontWeight: '600', padding: '10px 20px', borderRadius: '8px', border: 'none', cursor: 'pointer' }}>수정</button>
              <button onClick={handleDelete} style={{ backgroundColor: '#fce4ec', color: '#e57373', fontWeight: '600', padding: '10px 20px', borderRadius: '8px', border: 'none', cursor: 'pointer' }}>삭제</button>
            </>
          )}
        </div>
      </div>
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: '20px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)', padding: isMobile ? '20px' : '40px', marginTop: '16px', overflow: 'hidden' }}>
        ...
        {user ? (
          <div style={{ display: 'flex', gap: '12px', marginBottom: '32px', alignItems: 'flex-start' }}>
            {getAvatar(profile?.nickname || user.email, profile?.avatar_url)}
            <div style={{ flex: 1 }}>
              <input value={commentInput} onChange={e => setCommentInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCommentSubmit()} placeholder="댓글 추가..." style={{ width: '100%', padding: '8px 0', outline: 'none', fontSize: '14px', boxSizing: 'border-box', border: 'none', borderBottom: '2px solid #e0e0e0', backgroundColor: 'transparent' }} />
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px' }}>
