...
function PostForm() {
  ...
  const uploadImage = async () => {
    ...
    if (!session) { alert('로그인이 필요합니다.'); return null; }
  ...
  const imageHandler = () => {
    ...
    input.onchange = async () => {
      ...
      const fileName = `content/${Date.now()}_${Math.random().toString(36).substring(2)}.${file.name.split('.').pop()}`;
      const { error } = await supabase.storage
        .from('board-images')
  ...
  // ✅ useMemo로 modules 정의 (imageHandler 포함)
  ...
  const handleSubmit = async () => {
    ...
    if (isEdit) {
      api.put(`/posts/${id}`, postData).then(() =>
        navigate(`/posts/${id}${fromAdmin ? '?from=admin&tab=posts' : categoryParam}`)
      );
    } else {
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: '20px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)', padding: '40px' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          <label style={{ fontSize: '14px', fontWeight: '600', color: '#666', display: 'block', marginBottom: '8px' }}>
            대표 이미지 첨부 <span style={{ fontWeight: '400', color: '#aaa' }}>(썸네일용)</span>
          </label>
